...
function getConfig(request) {
  ...
  var optMethodURL1 = config.newOptionBuilder().setLabel('metrics.chats').setValue('https://api.tawk.to/v1/metrics.chats');
  var optMethodURL2 = config.newOptionBuilder().setLabel('metrics.tickets-new').setValue('https://api.tawk.to/v1/metrics.tickets-new');
  var optMethodURL3 = config.newOptionBuilder().setLabel('metrics.tickets-reopened').setValue('https://api.tawk.to/v1/metrics.tickets-reopened');
  var optMethodURL4 = config.newOptionBuilder().setLabel('metrics.tickets-resolved').setValue('https://api.tawk.to/v1/metrics.tickets-resolved');
  var optMethodURL5 = config.newOptionBuilder().setLabel('metrics.tickets-responses').setValue('https://api.tawk.to/v1/metrics.tickets-responses');
  var optMethodURL6 = config.newOptionBuilder().setLabel('chat.list').setValue('https://api.tawk.to/v1/chat.list');
  ...
  config
    ...
    .setId('urlMethod')
    .setName('Metrics API')
    .setHelpText('Which one of the tawk.to metrics do you want to query?')
    .setAllowOverride(true)
    .addOption(optMethodURL1).addOption(optMethodURL2).addOption(optMethodURL3).addOption(optMethodURL4).addOption(optMethodURL5).addOption(optMethodURL6);

  config
    .newTextInput()
    .setId('propertyId')
    .setName('Property ID')
    .setHelpText('Property ID of your tawk.to property. Found under Administration.')
    .setPlaceholder('');
  ...
  config
    ...
    .setHelpText('Found under your profile. However you have to beg tawk.to support for access.')
    .setPlaceholder('');
 ...
 */
function fetchJSON(propertyId, apiKey, url) {
  console.log('fetchJSON url: '+url);
  console.log('fetchJSON propertyId: '+propertyId);
  console.log('fetchJSON apiKey: '+apiKey);
  ...
  try {
    var payload = {
      ...
      'query': {
        'startTime': '2024-01-01T00:00:22Z',
        'endTime': '2024-12-31T00:00:22Z',
        'granularity': 'day'
      }
    }
    var headers = {
    ...
    console.log('fetchJSON options: '+JSON.stringify(options));
  } catch (e) {
 ...
 * @returns {Object}      The response object
 */
function getCachedData(propertyId, apiKey, url) {
...
/*  if (cacheKeyString !== null && cacheKeys.length > 0) {
    ...
    content = fetchJSON(propertyId, apiKey, url);
 ...
 * @returns {Object}        The response object
 */
function fetchData(propertyId, apiKey, url, cache) {
  if (!url || !url.match(/^https?:\/\/.+$/g)) {
    sendUserError('"' + url + '" is not a valid url.');
  }
  try {
    //var content = cache ? getCachedData(propertyId, apiKey, url) : fetchJSON(propertyId, apiKey, url);
    var content = fetchJSON(propertyId, apiKey, url);
  } catch (e) {
    sendUserError(
  ...
  console.log('fetchData propertyId: '+propertyId);
  console.log('fetchData apiKey: '+apiKey);
  console.log('fetchData content: '+JSON.stringify(content));
  //console.log('fetchData data: '+content["data"]);
  //console.log('fetchData data.metrics: '+content["data"]["metrics"]);
  //console.log('fetchData data.metrics: '+content[0][0]);
...
function getSchema(request) {
  var propertyId = request.configParams.propertyId;
  var apiKey = request.configParams.apiKey;
  var content = fetchData(propertyId, apiKey, request.configParams.urlMethod, request.configParams.cache);
